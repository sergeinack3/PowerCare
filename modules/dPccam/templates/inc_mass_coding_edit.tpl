      ...
      {{foreach from=$_activite->phases item=_phase}}
        ...
        {{if (!$acte->_id || ($acte->executant_id == $codage->praticien_id && $acte->_id|@array_key_exists:$codage->_ref_actes_ccam)) &&
          ...
          {{if $display_code}}
            ...
            <tr>
              <th class="section" colspan="14" style="text-align: left;">
                ...
                {{if $_code->forfait}}
                  <span class="circled" title="{{tr}}CDatedCodeCCAM.forfait.{{$_code->forfait}}-desc{{/tr}}" style="color: firebrick; border-color: firebrick;">
          ...
          <tr {{if !$acte->_id}}class="activite-{{$acte->code_activite}}"{{/if}}>
              ...
              {{foreach from=$_phase->_modificateurs item=_mod name=modificateurs}}
                <span class="circled {{if $_mod->_state == 'prechecked'}}ok{{elseif $_mod->_checked && in_array($_mod->_state, array('not_recommended', 'forbidden')) || $_mod->_montant == '0'}}error{{elseif in_array($_mod->_state, array('not_recommended', 'forbidden'))}}warning{{/if}}"
                  ...
                  <input type="checkbox" name="modificateur_{{$_mod->code}}{{$_mod->_double}}"
                         ...
                         data-acte="{{$view}}" data-code="{{$_mod->code}}" data-price="{{$_mod->_montant}}" data-double="{{$_mod->_double}}" class="modificateur" onchange="syncCodageField(this, '{{$view}}');" />
            ...
            <td>
              <form name="codageActe-{{$view}}" action="?" method="post" data-view="{{$view}}" onsubmit="return submitFormAct(this);">
                ...
                {{else}}
                  <button class="edit notext compact" type="button" onclick="ActesCCAM.edit({{$acte->_id}}, {onClose: refreshMassCoding.curry()})">{{tr}}Edit{{/tr}}</button>
                  <button class="remove notext compact" type="button"
                          onclick="confirmDeletion(this.form,{typeName:'l\'acte',objName:'{{$acte->_view|smarty:nodefaults|JSAttribute}}', ajax: '1'},
                            {onComplete: refreshMassCoding.curry()});">
                    {{tr}}Remove{{/tr}}
                  </button>
                {{/if}}
              </form>
...
<div id="concreteRules_codage-{{$codage->_id}}" style="display: none;">
  <form name="formCodageRules_codage-{{$codage->_id}}" action="?" method="post"
        onsubmit="return onSubmitFormAjax(this, {onComplete: function() {refreshMassCoding.curry()}});">
