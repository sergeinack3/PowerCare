...
<head>
    <style>
        ...
        @media print {
          ...
          table.main > tbody > tr > td > fieldset:not([class^='v-']) {
          ...
          table.tbl > tbody > tr > th:last-of-type,
          ...
          table.tbl > tbody > tr:last-of-type td {
            border-bottom: none !important;
          }

          #printDevisPdf,
          #printDevisPdf th.title,
          #infosPraticien,
          #infosPatient,
          #infosTotal,
          #infosActes {
          ...
          table#printDevisPdf th.title,
          table#printDevisPdf td.title {
...
<body>
<table id="printDevisPdf" class="main tbl form me-border-radius me-no-box-shadow">
    <tr>
    ...
    <tr>
        <td class="halfpane me-border-right-0 me-valign-top" id="infosPraticien">
            <fieldset>
                ...
                <table class="tbl">
                    <tr>
                        ...
                        <td class="me-no-border me-text-align-left">
                            {{mb_value object=$praticien field=_user_last_name}}
                            {{mb_value object=$praticien field=_user_first_name}}
                        </td>
                    </tr>
                    {{if $praticien->adeli || $praticien->rpps}}
                        {{mb_ternary var=idnat test=$praticien->adeli value=$praticien->adeli other=$praticien->rpps}}
                        <tr>
                            <td class="me-no-border me-text-align-left">N° {{tr}}common-ID{{/tr}} :</td>
                            <td class="me-no-border me-text-align-left">{{$idnat}}</td>
                        </tr>
                    {{/if}}
                    {{if $praticien->_ref_function}}
                        ...
                        {{if $function->adresse && $function->ville && $function->cp}}
                            <tr>
                                <td class="me-no-border me-text-align-left">{{tr}}common-Address{{/tr}} :</td>
                                <td class="me-no-border me-text-align-left">{{$function->adresse}} {{$function->cp}}
                                    {{$function->ville}}</td>
                            </tr>
                        {{/if}}
                        {{if $function->tel}}
                            <tr>
                                <td class="me-no-border me-text-align-left">{{tr}}common-Phone{{/tr}} :</td>
                                <td class="me-no-border me-text-align-left">{{$function->tel}}</td>
                            </tr>
                        {{/if}}
                    {{/if}}
                </table>
        ...
        <td class="halfPane me-border-right-0 me-valign-top" id="infosPatient">
            <fieldset>
                {{assign var=patient value=$devis->_ref_patient}}
                <legend>{{tr}}common-Patient{{/tr}}</legend>
                <table class="tbl">
                    <tr>
                        <td class="me-no-border me-text-align-left">{{tr}}common-Name{{/tr}} :</td>
                        <td
                          class="me-text-align-left">{{mb_value object=$patient field=nom}} {{mb_value object=$patient field=prenom}}</td>
                    </tr>
                    <tr>
                        <td class="me-no-border me-text-align-left">{{tr}}common-Address{{/tr}} :</td>
                        <td
                          class="me-no-border me-text-align-left">{{$patient->adresse}} {{$patient->cp}} {{$patient->ville}}</td>
                    </tr>
                    <tr>
                        <td class="me-no-border me-text-align-left">{{tr}}common-Phone{{/tr}} :</td>
                        <td class="me-no-border me-text-align-left">{{$patient->tel}}</td>
                    </tr>
                </table>
            </fieldset>
        </td>
        ...
        <td colspan="2" id="infosActes">
            <table class="tbl form">
                {{if $devis->_ref_actes_ccam|@count != 0}}
                    <tr>
                        <th colspan="8" class="me-text-align-left title">{{tr}}CActeCCAM{{/tr}}</th>
                    </tr>
                    <tr>
                        <th class="narrow"></th>
                    ...
                    {{foreach from=$devis->_ref_actes_ccam item=_acte}}
                        <tr>
                            <td class="me-no-border me-text-align-center"></td>
                            <td class="me-no-border me-text-align-center">
                                {{mb_value object=$_acte field=code_acte}}
                            </td>
                            <td class="me-no-border me-text-align-center">
                                {{mb_value object=$_acte field=code_activite}}
                                - {{mb_value object=$_acte field=code_phase}}
                            </td>
                            <td class="me-no-border me-text-align-center">
                                {{mb_value object=$_acte field=modificateurs}}
                            </td>
                            <td class="me-no-border me-text-align-right">
                                {{mb_value object=$_acte field=montant_base}}
                            </td>
                            <td class="me-no-border me-text-align-right">
                                {{mb_value object=$_acte field=montant_depassement}}
                            </td>
                            <td class="me-no-border me-text-align-right">-</td>
                            <td class="me-no-border me-text-align-right">
                ...
                {{if $devis->_ref_actes_ngap|@count != 0}}
                    <tr>
                        <th colspan="8" class="me-text-align-left title">{{tr}}CActeNGAP|pl{{/tr}}</th>
                    </tr>
                    <tr>
                ...
                {{if $devis->_ref_actes_lpp|@count != 0}}
                    <tr>
                        <th colspan="8" class="me-text-align-left title">{{tr}}CActeLPP|pl{{/tr}}</th>
                    </tr>
                    <tr>
                ...
                {{if $devis->_ref_frais_divers|@count != 0}}
                    <tr>
                        <th colspan="8" class="me-text-align-left title">{{tr}}CFraisDivers{{/tr}}</th>
                    </tr>
                    <tr>
                    ...
                    {{foreach from=$devis->_ref_frais_divers item=_frais}}
                        <tr>
                            <td class="me-no-border me-text-align-center">
                                {{mb_value object=$_frais field=quantite}}
                            </td>
                            <td class="me-no-border me-text-align-center"></td>
                            <td class="me-no-border me-text-align-center">
                                {{mb_value object=$_frais->_ref_type field=libelle}}
                            </td>
                            <td class="me-no-border me-text-align-center"
                            {{mb_value object=$_frais field=coefficient}}
                            </td>
                            <td class="me-no-border me-text-align-right">-</td>
                            <td class="me-no-border me-text-align-right">-</td>
                            <td class="me-no-border me-text-align-right">
                                {{mb_value object=$_frais field=montant_base}}
                            </td>
                            <td class="me-no-border me-text-align-right">
                                {{mb_value object=$_frais field=montant_base}}
                            </td>
