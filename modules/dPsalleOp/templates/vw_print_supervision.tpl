...
{{foreach from=$surveillance_data item=_data key=type}}
  ...
  <script>
    Main.add(function(){
      ...
      if (Object.isUndefined(window.surveillances)) {
        window.surveillances = $H();
      }

      window.surveillances.set('{{$type}}', new SurveillanceTimeline(
  ...
  <table class="main print" style="page-break-inside: avoid;">
    ...
    <tr>
      <td>
        <div style="position: relative;" class="supervision supervision-{{$type}} surveillance-timeline-container" id="surveillance-timeline-{{$type}}">
          ...
          {{foreach from=$_data.graphs item=_graph key=i}}
            {{if $_graph|instanceof:'Ox\Mediboard\MonitoringPatient\CSupervisionGraph'}}
              ...
              <div id="placeholder-{{$type}}-{{$i}}" style="height:{{$_graph->height}}px; width: 100%;"
              ...
              <script>
                ...
                Main.add(function() {
                  ...
                  var graph = {
                    key: '{{$_graph->_guid}}-{{$timeline_uid}}-{{$type}}',
                  ...
                  var surveillance = window.surveillances.get('{{$type}}');

                  ph.bind("plotpan", function (event, plot) {
                    surveillance.updateOffsetsFromFlot.bind(surveillance)(plot);
                  });
                  ...
                  surveillance.append(graph.key, graph);
                });
              </script>
            {{elseif $_graph|instanceof:'Ox\Mediboard\MonitoringPatient\CSupervisionTimeline'}}
              ...
              <script>
                Main.add(function() {
                  ...
                  surveillance.applyOffsets(false);
                });
              </script>
                ...
                Main.add(function() {
                  ...
                  App.loadJS(['lib/visjs/vis'], (function(currentTL, vis) {
                    ...
                    var stli = new SurveillanceTimelineItem(
                      vis,
                      currentTL,
                      '{{$_graph->getIdentifier()}}-{{$timeline_uid}}',
                      $("timeline-{{$timeline_uid}}"),
                      groups,
                      data,
