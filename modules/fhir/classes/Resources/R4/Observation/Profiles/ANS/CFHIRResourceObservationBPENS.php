...
use Ox\Interop\Fhir\Resources\R4\Observation\Mapper\ANS\ENSObservationBP;
use Ox\Mediboard\Patients\Constants\CValueInt;

/**
 * Description
 */
class CFHIRResourceObservationBPENS extends CFHIRResourceObservation
{
    /** @var string */
    public const PROFILE_TYPE = 'ENS_FrObservationBp';
    ...
    protected function setMapperOld(CStoredObject $object): DelegatedObjectMapperInterface
    {
        ...
        if ($ref_spec->code !== 'systole' && $ref_spec->code !== 'diastole') {
            throw new CFHIRExceptionNotSupported("Abstract constant is not a systole or a diastole");
        }

        $mapping_object = ENSObservationBP::class;
