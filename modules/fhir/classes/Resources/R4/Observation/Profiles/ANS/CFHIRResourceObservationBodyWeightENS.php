...
use Ox\Interop\Fhir\Resources\R4\Observation\Mapper\ANS\ENSObservationBodyWeight;
use Ox\Mediboard\Patients\Constants\CValueInt;

/**
 * Description
 */
class CFHIRResourceObservationBodyWeightENS extends CFHIRResourceObservation
{
    // constants
    /** @var string */
    public const PROFILE_TYPE = 'ENS_FrObservationBodyWeight';
    ...
    protected function setMapperOld(CStoredObject $object): DelegatedObjectMapperInterface
    {
        ...
        if ($ref_spec->code !== 'weight') {
            throw new CFHIRExceptionNotSupported("Abstract constant is not a weight");
        }

        $mapping_object = ENSObservationBodyWeight::class;
