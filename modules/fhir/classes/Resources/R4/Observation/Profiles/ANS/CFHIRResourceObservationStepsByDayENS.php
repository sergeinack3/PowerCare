...
use Ox\Interop\Fhir\Resources\R4\Observation\Mapper\ANS\ENSObservationStepByDay;
use Ox\Mediboard\Patients\Constants\CValueInt;

/**
 * Description
 */
class CFHIRResourceObservationStepsByDayENS extends CFHIRResourceObservation
{
    // constants
    /** @var string */
    public const PROFILE_TYPE = 'ENS_ObservationStepsByDay';
    ...
    protected function setMapperOld(CStoredObject $object): DelegatedObjectMapperInterface
    {
        ...
        if ($ref_spec->code !== 'dailyactivity') {
            throw new CFHIRExceptionNotSupported("Abstract constant is not a daily activity");
        }

        $mapping_object = ENSObservationStepByDay::class;
