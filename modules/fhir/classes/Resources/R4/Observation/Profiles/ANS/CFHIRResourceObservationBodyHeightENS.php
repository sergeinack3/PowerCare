...
use Ox\Interop\Fhir\Resources\R4\Observation\Mapper\ANS\ENSObservationBodyHeight;
use Ox\Mediboard\Patients\Constants\CValueInt;

/**
 * Description
 */
class CFHIRResourceObservationBodyHeightENS extends CFHIRResourceObservation
{
    /** @var string */
    public const PROFILE_TYPE = 'ENS_FrObservationBodyHeight';
    ...
    protected function setMapperOld(CStoredObject $object): DelegatedObjectMapperInterface
    {
        ...
        if ($ref_spec->code !== 'height') {
            throw new CFHIRExceptionNotSupported("Abstract constant is not a height");
        }

        $mapping_object = ENSObservationBodyHeight::class;
