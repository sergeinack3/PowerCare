...
use Ox\Interop\Fhir\Resources\R4\Observation\Mapper\ANS\ENSObservationHeartrate;
use Ox\Mediboard\Patients\Constants\CValueInt;

/**
 * Description
 */
class CFHIRResourceObservationHeartrateENS extends CFHIRResourceObservation
{
    // constants
    /** @var string */
    public const PROFILE_TYPE = 'ENS_FrObservationHeartrate';
    ...
    protected function setMapperOld(CStoredObject $object): DelegatedObjectMapperInterface
    {
        ...
        if ($ref_spec->code !== 'heartrate') {
            throw new CFHIRExceptionNotSupported("Abstract constant is not a heartrate");
        }

        $mapping_object = ENSObservationHeartrate::class;
