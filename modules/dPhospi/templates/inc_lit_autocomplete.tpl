...
<ul>
  {{foreach from=$matches item=match}}
    <li id="autocomplete-{{$match->_guid}}"
        ...
        data-chambre-id="{{$match->_ref_chambre->_id}}" data-service-id="{{$match->_ref_chambre->_ref_service->_id}}"
        data-chambre-nom="{{$match->_ref_chambre->nom}}"
        style="{{if $match->_occupe}}background-color: #fc0{{/if}}">
