...
$column_name = CValue::post("column");
...
$column = $xpath->query("column[@name='$column_name']", $table)->item(0);
if (!$column) {
  $column = $dom->createElement('column');
  $column->setAttribute('name', $column_name);

  $table->appendChild($column);
}

$column->setAttribute($type, utf8_encode($value));
