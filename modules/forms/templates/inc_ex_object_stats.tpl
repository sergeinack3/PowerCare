...
<script>
  Main.add(function () {
    {{foreach from=$series key=_key item=_series}}
      plotExObject('{{$_key}}', {{$_series.series|@json}}, {{$_series.options|@json}}, {{$total.$_key|@json}});
    {{/foreach}}
  });

  plotExObject = function (key, series, options, total) {
    ...
    function plotHoverExObject(event, pos, item) {
      if (item) {
        ...
        var content = printf("<strong>%s</strong><br />%d / %d<br />%s",
          date,
