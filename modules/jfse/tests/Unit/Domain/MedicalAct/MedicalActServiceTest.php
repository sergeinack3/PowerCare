...
namespace Ox\Mediboard\Jfse\Tests\Unit\Domain\MedicalAct;
...
class MedicalActServiceTest extends UnitTestJfse
{
    /** @var MockObject */
    private $client;

    public function testSetMedicalAct(): void
    {
        $fse_response = <<<JSON
{
    "method": {
...
JSON;
        ...
        $this->client = $this->makeClientFromGuzzleResponses(
            [$this->makeJsonGuzzleResponse(200, utf8_encode($fse_response))]
        );

        $act = new CActeNGAP();
        $act->code = 'C';
        $act->execution = '2020-12-01 16:00:00';
        $act->quantite = 1;
        $act->coefficient = 1;
        $act->montant_base = 23.0;
        $actual       = (new MedicalActService(new MedicalActClient($this->client)))->setMedicalAct(
            "1606833898504621498",
            $act
        );
