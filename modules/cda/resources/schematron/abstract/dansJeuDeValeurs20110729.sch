...
<pattern xmlns="http://purl.oclc.org/dsdl/schematron" id="dansJeuDeValeurs" abstract="true" 
    ...
    <rule context="$xpath_elt">
        ...
        <assert test="(not(@nullFlavor) or $nullFlavor)">
            Erreur de conformité CI-SIS : L'élément "<value-of select="$vue_elt"/>" ne doit pas comporter d'attribut nullFlavor.
        </assert>
        <assert test="(
            (@code and @codeSystem and @displayName) or
            ($nullFlavor and 
            (@nullFlavor='UNK' or @nullFlavor='NASK' or @nullFlavor='ASKU' or @nullFlavor='NAV' or @nullFlavor='MSK')) or
            (@xsi:type and not(@xsi:type = 'CD') and not(@xsi:type = 'CE'))
            )">
            Erreur de conformité CI-SIS : L'élément "<value-of select="$vue_elt"/>" doit avoir ses attributs 
        ...
        -->
            Erreur de conformité CI-SIS : L'élément <value-of select="$vue_elt"/>
