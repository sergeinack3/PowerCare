...
/**
 * Class PrescriptionEndCategorySoins
 */
class PrescriptionEndCategorySoins extends TimelineCategory implements ITimelineCategory
{
        ...
        if ($prescription->_id) {
            ...
            foreach ($prescription->_ref_prescription_lines_element as $_line) {
                ...
                if ($this->selectedPractitioner($_line->_ref_praticien)) {
                    $dates_list = $this->makeListDates($_line->_fin_reelle);
                    $this->appendTimeline($dates_list[0], $dates_list[1], $dates_list[2], 'prescription_end', $_line);
            ...
            foreach ($prescription->_ref_prescription_line_mixes as $_line) {
                ...
                if ($this->selectedPractitioner($_line->_ref_praticien)) {
                    $dates_list = $this->makeListDates($_line->_fin);
                    $this->appendTimeline($dates_list[0], $dates_list[1], $dates_list[2], 'prescription_end', $_line);
                    $this->addToInvolvedUser($_line->_ref_praticien);
                }
            }
