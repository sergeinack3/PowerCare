        ...
        if ($praticien_id && $only_non_checked) {
            ...
            if ($service_id || count($services_ids) || $praticien_id || $function_id || $discipline_id) {
                ...
                foreach ($affectations as $_affectation) {
                    ...
                    $sejour->loadRefCurrAffectation()->loadRefLit()->loadCompleteView();
...
if ($sejour_id) {
    ...
    if ($validation_pharm) {
        $smarty->assign("count_validation_pharma", $count_validation_pharma);
    }
    ...
    $smarty->display("inc_vw_sejours_global");
} else {
    // Affichage de la liste des sejours
    $smarty->display("vwSejours");
}
