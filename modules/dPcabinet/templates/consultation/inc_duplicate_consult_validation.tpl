    ...
    <div style="text-align:center; white-space: initial" class="small-warning">
        ...
        {{$_consultation}}
                ...
                <button type="button" class="right notext"
                    ...
                    {{tr}}CConsultation-action-Duplicate{{/tr}}
                </button>
            </div>
        {{/foreach}}
    </div>
    <button type="button" class="duplicate"
            onclick="if (confirm($T('CConsultation.Duplicate consultation'))) { this.form.onsubmit();}">
        {{tr}}CConsultation-action-Duplicate{{/tr}}
    </button>
{{else}}
    <button type="submit" class="duplicate">{{tr}}CConsultation-action-Duplicate{{/tr}}</button>
{{/if}}
