...
class FourDDump {
  ...
  private function newThreads($table_name = NULL) {
    ...
    do {
      ...
      passthru('php ' . $_SERVER['PHP_SELF'] . $this->renderArgs());
  ...
  function parseColumn($fourd_column) {
    ...
    $column_name = $fourd_column['COLUMN_NAME'];
  ...
  function dumpTableStructure($table) {
    ...
    // MB additional column for importing purposes
    $lines[] = PHP_EOL . "  `to_skip` ENUM('0', '1') NOT NULL DEFAULT '0'";
    ...
    print(PHP_EOL . ') ENGINE=MyISAM DEFAULT CHARSET=utf8;');
    print(PHP_EOL);
  }
  ...
  function dumpTableData($table) {
    ...
    while ($row = $this->fourd->getRow()) {
      $values = array();
      foreach ($table->columns as $column) {
        $value = utf8_decode($row[mb_strtoupper($column->original_name, 'UTF-8')]);
        switch ($column->type) {
          case 'bool':
      ...
      // MB additional value (to_skip) for importing purposes
      $values[] = "'0'";
