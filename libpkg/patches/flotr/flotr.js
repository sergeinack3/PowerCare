...
Flotr.Graph = Class.create({
  ...
  getEventPosition: function (event){
    var offset = this.overlay.cumulativeOffset(),
        scroll = this.overlay.cumulativeScrollOffset(),
        pointer = Event.pointer(event),
        docElement = document.documentElement,
        body = document.body;
        
    offset.top  -= scroll.top  - (docElement.scrollTop  || body.scrollTop  || 0);
    offset.left -= scroll.left - (docElement.scrollLeft || body.scrollLeft || 0);
        
    var rx = (pointer.x - offset.left - this.plotOffset.left),
  ...
  clearHit: function(){
    ...
    if(!s.bars.show && !s.pie.show && !s.bubbles.show){
      var offset = s.mouse.radius + lw * 4;
      this.octx.clearRect(
      ...
      if(!s.bars.horizontal){ // vertical bars (default)
        ...
        if(s.bars.centered) {
          this.octx.clearRect(
              plotOffset.left + xa.d2p(prevHit.x - bw/2) - offset, 
              plotOffset.top  + lastY - offset, 
              xa.d2p(bw + xa.min) + offset * 2, 
              ya.d2p(prevHit.y < 0 ? prevHit.y : 0) - lastY + offset * 2
          );
        } else {
          this.octx.clearRect(
              plotOffset.left + xa.d2p(prevHit.x) - offset, 
        ...
        if(s.bars.centered) {
          this.octx.clearRect(
              lastX + plotOffset.left + offset, 
              ya.d2p(prevHit.y + bw/2) + plotOffset.top - offset, 
              xa.d2p(prevHit.x < 0 ? prevHit.x : 0) - lastX - offset * 2,
              ya.d2p(bw + ya.min) + offset * 2
          );
        } else {
          this.octx.clearRect(
              lastX + plotOffset.left + offset, 
              ya.d2p(prevHit.y + bw) + plotOffset.top - offset, 
              xa.d2p(prevHit.x < 0 ? prevHit.x : 0) - lastX - offset * 2,
              ya.d2p(bw + ya.min) + offset * 2
          );
        }
...
Flotr.addType('markers', {
  ...
  extendYRange: function(axis){
    if (axis.options.max == null){
      ...
      for(i = 0; i < this.series.length; ++i){
        c = this.series[i].markers;
        if(c && c.show && this.series[i].yaxis == axis) {
          data = this.series[i].data;
          if (data)
            for(j = 0; j < data.length; j++) {
              d = data[j];
              newmax = Math.max(d[1] * (1 + (axis.options.autoscaleMargin || 5) / 100), newmax);
              newmin = Math.min(d[1] * (1 + (axis.options.autoscaleMargin || 5) / 100), newmin);
            }
        }
      }
      axis.max = newmax;
...
Flotr.addPlugin('spreadsheet', {
  ...
  constructDataGrid: function(){
    ...
    var toolbar = new Element('div').addClassName('flotr-datagrid-toolbar').
      insert(
        new Element('a', {id: 'flotr-datagrid-toolbar-download'})
            .addClassName('button flotr-datagrid-toolbar-button')
  ...
  downloadCSV: function(){
    ...
    /**
     * Some browsers don't handle the download correctly
     *
     * MSIE : Detect IE 10 and previous
     * Trident/ : Detect IE 11 and next
     * Edge/ : Detect Edge
     */
    var user_agent = navigator.userAgent;
    if (user_agent.indexOf('MSIE ') != -1 || user_agent.indexOf('Trident/') != -1 || user_agent.indexOf('Edge/') != -1) {
      csv = csv.replace(new RegExp(separator, 'g'), decodeURIComponent(separator)).replace(/%0A/g, '\n').replace(/%0D/g, '\r');
      window.open().document.write(csv);
    }
    else {
      var a = $('flotr-datagrid-toolbar-download');
      a.href = 'data:text/csv,'+csv;
      a.download = this.options.title + '.csv';
    }
  }
});
